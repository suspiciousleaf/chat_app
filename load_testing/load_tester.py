...
class LoadTester:
    ...
    def __init__(
        ...
        self.monitor: Monitor = Monitor(
            self.logger, account={"username": MONITOR_USER, "password": MONITOR_PASS}
        )
    ...
    async def create_and_run_user(self, account):
        ...
        user = User(
            ...
            delay_before_actions=self.delay_before_actions,
    ...
    async def run_load_test(self):
        ...
        )  # sorted() used to convert dict to sequence
        tasks = []
        tasks.append(asyncio.create_task(self.monitor.run()))
        for i, account in enumerate(accounts_to_use):
            account = {"username": account, "password": self.test_account_pool[account]}
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.info("LoadTester complete")
            ...
            try:
                ...
                self.logger.warning(
                    f"Unable to process performance data: {type(e).__name__}:{e}"
                )
        ...
        if self.monitor.perf_data:
            ...
            for data_point in self.monitor.perf_data.values():
                try:
                    ...
                    if (
                        max(data_point["cpu_load"]) < 3
                        or not data_point["active_connections"]
                    ):
                        continue
            ...
            axs[0, 1].scatter(
                perf_test_id, message_volume, color="#008fde", label="Message Volume"
            )
            ...
            num_bins = 10
            bins = np.linspace(
                min(message_volume), max(message_volume), num_bins + 1
            )  # num_bins + 1 to account for bin edges

            latency_groups = [
                latency[(message_volume >= bins[i]) & (message_volume < bins[i + 1])]
                for i in range(num_bins)
            ]
            latency_groups[-1] = latency[
                (message_volume >= bins[-2]) & (message_volume <= bins[-1])
            ]
            ...
            # Save the figure
            plt.savefig(
                f"perf_data/{file_name}.png", dpi=300
