...
def ping():
    db_status = connection_man.db.verify_connection_and_tables()
    redis_status = connection_man.redis_man.verify_connection()
    if db_status.get("status") and redis_status.get("status"):
        return {"status": "ready"}
    else:
        statuses = []
        if db_status.get("status") == False:
            statuses.append(db_status.get("detail"))
        if redis_status.get("status") == False:
            statuses.append(redis_status.get("detail"))
